...
const SimpleKitchen = () => {
  ...
  const [showKitchenNoteEditModal, setShowKitchenNoteEditModal] = useState(false)
      ...
      setOrders([
        {
          ...
          priority: 'normal',
          kitchen_notes: 'Customer has nut allergy',
          order_type: 'dine_in'
        },
        {
          id: 2,
        ...
        },
        {
          ...
          priority: 'urgent',
          kitchen_notes: 'Extra sauce on the side',
          order_type: 'takeout'
        },
        {
          id: 4,
  ...
  const saveOrderEdits = () => {
    ...
    setOrders(prev => prev.map(order => 
      order.id === editingOrder.id 
        ? { 
            ...order,
            table: orderEditForm.table,
            customer: orderEditForm.customer,
            special_instructions: orderEditForm.special_instructions,
            kitchen_notes: orderEditForm.kitchen_notes,
            priority: orderEditForm.priority,
            order_type: orderEditForm.order_type,
  ...
  const openNoteEditModal = (note?: KitchenNote) => {
    if (note) {
      setEditingNote(note)
      setNoteForm({
      ...
      setShowKitchenNoteEditModal(true)
    } else {
      setEditingNote(null)
  ...
  const saveNoteEdit = () => {
    ...
    setShowKitchenNoteEditModal(false)
    setEditingNote(null)
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="mt-3">
              ...
              <div className="space-y-4 max-h-96 overflow-y-auto">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Table/Order ID</label>
                    <input
                      type="text"
                      value={orderEditForm.table}
                      onChange={(e) => setOrderEditForm({...orderEditForm, table: e.target.value})}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      value={orderEditForm.priority}
                      onChange={(e) => setOrderEditForm({...orderEditForm, priority: e.target.value as Order['priority']})}
                      className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                  ...
                  <div>
                    ...
                    <select
                      value={orderEditForm.order_type}
                      onChange={(e) => setOrderEditForm({...orderEditForm, order_type: e.target.value as Order['order_type']})}
                      className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onKeyPress={(e) => e.key === 'Enter' && addItemToOrder()}
                    />
                    <button
                ...
                <div>
                  ...
                  <textarea
                    value={orderEditForm.special_instructions}
                    onChange={(e) => setOrderEditForm({...orderEditForm, special_instructions: e.target.value})}
                    rows={2}
                    className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">Kitchen Notes</label>
                  <textarea
                    value={orderEditForm.kitchen_notes}
                    onChange={(e) => setOrderEditForm({...orderEditForm, kitchen_notes: e.target.value})}
                    rows={2}
                    className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="Internal kitchen notes..."
                  />
                </div>
              </div>
      ...
      {showKitchenNoteEditModal && editingNote && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Edit Kitchen Note</h3>
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button
                  onClick={() => setShowKitchenNoteEditModal(false)}
                  className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors"
                >
                  Cancel
                </button>
